...
const AdminDashboard = () => {
  ...
  useEffect(() => {
    ...
    if (path === 'courses') setActiveTab('courses');
    else setActiveTab('creators');
  }, [location]);
...
const CreatorApplications = () => {
  ...
  const fetchCreators = async () => {
    try {
      const response = await api.get('/admin/creators'); // updated route
  ...
  const handleApproval = async (creatorId, action) => {
    try {
      await api.patch(`/admin/creator/${creatorId}/approve`, { action }); // updated route
  ...
  if (loading) return <div className="flex items-center justify-center py-12"><div className="animate-spin rounded-full h-32 w-32 border-b-2 border-primary-600"></div></div>;
            ...
            <div key={creator._id} className="card p-6">
              <div className="flex items-start justify-between">
                ...
                <div className="flex items-center space-x-2 ml-6">
                  <button onClick={() => handleApproval(creator._id, 'approve')} className="btn-primary text-sm">Approve</button>
                  <button onClick={() => handleApproval(creator._id, 'reject')} className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg text-sm">Reject</button>
                </div>
              </div>
            </div>
          ))}
...
const CourseReviews = () => {
  ...
  const fetchCourses = async () => {
    try {
      const response = await api.get(`/admin/courses?status=${filter}`); // updated route
  ...
  const handleCourseAction = async (courseId, action) => {
    try {
      await api.patch(`/admin/courses/${courseId}/publish`, { action }); // updated route
            ...
            <div key={course._id} className="card p-6">
              <div className="flex items-start justify-between">
                <div className="flex-1">
                  ...
                  <div className="text-xs text-gray-500">Submitted on {new Date(course.createdAt).toLocaleDateString()}</div>
                </div>
                <div className="flex items-center space-x-2 ml-6">
                  {course.status === 'pending' && (
                    <>
                      <button onClick={() => handleCourseAction(course._id, 'publish')} className="btn-primary text-sm">Publish</button>
                      <button onClick={() => handleCourseAction(course._id, 'reject')} className="bg-red-600 hover:bg-red-700 text-white px-4 py-2 rounded-lg text-sm">Reject</button>
                    </>
                  )}
                  {course.status === 'published' && <span className="text-green-600 text-sm font-medium">Published</span>}
                  {course.status === 'rejected' && <span className="text-red-600 text-sm font-medium">Rejected</span>}
                </div>
              </div>
            </div>
          ))}
